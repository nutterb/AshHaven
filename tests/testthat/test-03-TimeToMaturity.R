...
test_that(
  "Time to Maturity for Bamboo", 
  {
    ...
    expect_equal(round(time_to_maturity(1/3, 1, method = "mean"), 7), 
                 3.4133333)
    expect_equal(round(time_to_maturity(1/3, 1, p = .9, method = "quantile"), 3), 
                 14.872)
    ...
    expect_equal(round(time_to_maturity(1/3, 11, method = "mean"), 7), 
                 37.5466667)
    expect_equal(round(time_to_maturity(1/3, 11, p = .9, method = "quantile"), 3), 
                 163.595)
  }
)
...
test_that(
  "Time to Maturity for Wheat", 
  {
    ...
    expect_equal(round(time_to_maturity(1/3, 8, method = "mean"), 7), 
                 27.3066667)
    expect_equal(round(time_to_maturity(1/3, 8, p = .9, method = "quantile"), 3), 
                 118.978)
  }
)
